...
foreign key (categoria_idcategoria) references tb_categoria(id_categoria),
foreign key (autor_idautor) references tb_autor(id_autor)
);
...
change column autor_idautor fk_idautor int;

-- Chave estrangeira categoria

alter table tb_livro
change column categoria_idcategoria fk_idcategoria int;

alter table tb_livro
add constraint fk_idcategoria foreign key (fk_idcategoria) references tb_categoria (id_categoria);

alter table tb_livro
add constraint fk_idautor foreign key (fk_idautor) references tb_autor (id_autor);

alter table tb_livro
modify ano year;

alter table tb_livro
add ds_livro varchar(100);

alter table tb_autor
add nr_fone varchar(15);

alter table tb_categoria
change descricao ds_categoria varchar(150);

alter table tb_categoria
change nome nm_categoria varchar(45);
